...
#  用法：sudo bash awshk_c6in_2c_install.sh
#        sudo bash awshk_c6in_2c_install.sh --uninstall
# =============================================================
set -euo pipefail
...
VPS2_URL="http://69.12.74.54:43303/update"
...
configure_bbr() {
    log INFO "配置 BBR + fq（AWS HK c6in.large）..."
...
install_nyanpass() {
    ...
    log INFO "无人值守安装 nyanpass 实例${instance_num}：${service_name} (OPTIMIZE=${optimize})"
    install_cmd="S='${service_name}' OPTIMIZE=${optimize} timeout ${NYANPASS_TIMEOUT} bash <(curl -fLSs ${NYANPASS_INSTALL_URL}) rel_nodeclient \"${install_args}\""
