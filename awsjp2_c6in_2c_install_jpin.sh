...
NYANPASS_NAME="jpin"
NYANPASS_TOKEN="19fc68d8-4fee-4713-b8dd-890a76005b85"
...
install_nyanpass() {
    ...
    log INFO "安装 nyanpass 实例：${NYANPASS_NAME}"
    install_cmd="S=nyanpass OPTIMIZE=1 timeout ${NYANPASS_TIMEOUT} bash <(curl -fLSs ${NYANPASS_INSTALL_URL}) rel_nodeclient \"-t ${NYANPASS_TOKEN} -u ${NYANPASS_URL}\""

    if eval "$install_cmd" 2>&1 | tee -a "$LOG_FILE"; then
        log INFO "nyanpass 安装完成：${NYANPASS_NAME}"
    else
        log WARN "nyanpass 安装可能未完全成功：${NYANPASS_NAME}"
    fi
}
